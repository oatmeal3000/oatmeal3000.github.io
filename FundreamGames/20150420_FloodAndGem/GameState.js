function modifyTime(){
  var minute = localStorage.getItem(cookie_name);
  if( minute != null && minute != '0'){
    minute = Number(minute) - 1;
    localStorage.setItem(cookie_name, minute);
  } 
}
...
function timeCount(){
  ...
  minutes = localStorage.getItem(cookie_name);
  if( minutes == null){
    minutes = 0;
     localStorage.setItem(cookie_name, minutes);
  }else{
    minutes = Number(minutes) + 1;
    localStorage.setItem(cookie_name, minutes);
  } 
  document.getElementById('time_used').innerHTML= minutes + ' min.';
}
...
function GameState(world, playerPos, pickedStuff) {
  ...
  function evaluateGameState() {
    ...
    switch (thePlayerTile.toString()) {
      ...
      case "Tile.PlayerWon":
        gameStatus = "won";
        localStorage.setItem(level_name, gameStatus);
        break;
      default:
  ...
  function take(theTile) {
    ...
    if (theTile.gem()) {
      ...
      var sound_gem = document.getElementById('getgem').play();
    }
  ...
  function runWater() {
    ...
    if (needsUpdate) {
      defer();
      doNext(runWater);
      setTimeout(resume, 50); // XXX make this set up to match onscreen animation
    }
  }
