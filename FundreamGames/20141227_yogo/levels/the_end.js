yogo.levels.push({
    hints: [
        {
            text: "祝贺你，完成任务啦 :)",
            rect: {x: 0, y: 0, width: 601, height: 401}
        },
        {
            text: "B",
            rect: {x: 0, y: 0, width: 40, height: 40}
        },
        {
            text: "I",
            rect: {x: 40, y: 0, width: 40, height: 40}
        },
        {
            text: "E",
            rect: {x: 80, y: 0, width: 40, height: 40}
        },
        {
            text: "B",
            rect: {x: 120, y: 0, width: 40, height: 40}
        },
        {
            text: "E",
            rect: {x: 160, y: 0, width: 40, height: 40}
        },
        {
            text: "R",
            rect: {x: 200, y: 0, width: 40, height: 40}
        },
        {
            text: " ",
            rect: {x: 240, y: 0, width: 40, height: 40}
        },
        {
            text: "4",
            rect: {x: 280, y: 0, width: 40, height: 40}
        },
        {
            text: "E",
            rect: {x: 320, y: 0, width: 40, height: 40}
        },
        {
            text: "V",
            rect: {x: 360, y: 0, width: 40, height: 40}
        },
        {
            text: "A",
            rect: {x: 400, y: 0, width: 40, height: 40}
        },
        {
            text: "R",
            rect: {x: 440, y: 0, width: 40, height: 40}
        },
        {
            text: " ",
            rect: {x: 480, y: 0, width: 40, height: 40}
        },
        {
            text: "<",
            rect: {x: 520, y: 0, width: 40, height: 40}
        },
        {
            text: "3",
            rect: {x: 560, y: 0, width: 41, height: 40}
        }
    ],
    player: {
        position: {x: 300.5, y: 200.5}
    },
    exit: {
        position: {x: -100.5, y: -100.5}
    },
    bots: [],
    barriers: {
        emitters: [],
        reflectors: []
    }
});
