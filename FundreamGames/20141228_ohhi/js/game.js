...
var Game = new (function() {
  var self = this,
      ...
      currentPuzzle = null,
      checkTOH = 0,
      ojoos = ['太好了','太精彩了','神乎其神','很出色','卓越不凡','神奇出现啦','太惊人了','很棒','干的不错','也是极好的','真聪明','耀眼的明星啊','是在做梦么','太优秀了','真给力','超人啊','厉害','Ojoo','英明','令人崇拜','卓尔不群','太壮丽了','耶'],
      remainingOjoos = [],
      endGameTOH1,
      endGameTOH2,
      endGameTOH3,
  ...
  function showSizes() {
    onHomeScreen = false;
    showGame();
    $('#boardsize').html('<span>挑选一个棋盘大小</span>');
  ...
  function undo() {
    if (!undoStack.length) {
      ...
      if (!undone)
        grid.hint.show('那个是撤销按钮');
      else
        grid.hint.show('没啥可以撤销的');
      return;
    }
