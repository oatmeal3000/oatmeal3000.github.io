...
function showHelp() {
    $("#inst_main_body").html("一、积累的色块不超过六角形的灰色边框；<br><br>" + (settings.platform != 'mobile' ? '二、点击向左或向右方向键' : '二、触动屏幕的左边或右边') + "来旋转六角形；<br><br>三、相同色块超过三块可以被消除和得分；<br><br>" + (settings.platform != 'mobile' ? '四、按住向下方向键游戏速度可提升两倍。' : '四、触动里面的六角形游戏速度提升两倍。'));
    if (gameState == 1) {
        pause();
    }
